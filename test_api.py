...
async def test_start_round():
    ...
    async with httpx.AsyncClient() as client:
        ...
        return response.status_code == 200, round_data["round_id"]


async def test_post_event(round_id: str):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{API_BASE}/v1/rounds/{round_id}/events",
            headers=HEADERS,
            json=event_data
        )
...
async def test_task_runs(round_id: str):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{API_BASE}/v1/rounds/{round_id}/task-runs:batch-upsert",
            headers=HEADERS,
            json=task_runs_data
        )
...
async def test_agent_runs(round_id: str):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{API_BASE}/v1/rounds/{round_id}/agent-runs:upsert",
            headers=HEADERS,
...
async def test_weights(round_id: str):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.put(
            f"{API_BASE}/v1/rounds/{round_id}/weights",
            headers=HEADERS,
            json=weights_data
        )
...
async def test_finalize_round(round_id: str):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{API_BASE}/v1/rounds/{round_id}/finalize",
            headers=HEADERS,
            json=finalize_data
        )
...
async def test_get_status(round_id: str):
    """Test getting round status."""
    print("Testing get status...")
    
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{API_BASE}/v1/rounds/{round_id}/status?validator_uid=12",
            headers=HEADERS
        )
...
async def main():
    ...
    # Test start round
    success, round_id = await test_start_round()
    if not success:
    ...
    tests = [
        test_post_event(round_id),
        test_task_runs(round_id),
        test_agent_runs(round_id),
        test_weights(round_id),
        test_finalize_round(round_id),
        test_get_status(round_id)
    ]
