...
class APITester:
    ...
    async def test_round_start(self) -> str:
        ...
        return round_id if result["success"] else None
    
    async def test_generate_tasks(self, round_id: str):
        ...
        result = await self.make_request("POST", f"/v1/rounds/{round_id}/generate-tasks", data)
        self.log_test("Generate Tasks", result)
        return result["success"]
    
    async def test_distribute_tasks(self, round_id: str):
        """Test task distribution."""
        task_ids = [f"{round_id}_task_{i:04d}" for i in range(5)]
        ...
        result = await self.make_request("POST", f"/v1/rounds/{round_id}/distribute-tasks", data)
        self.log_test("Distribute Tasks", result)
        return result["success"]
    
    async def test_submit_responses(self, round_id: str):
        """Test submitting task responses."""
        responses = []
        for task_id in [f"{round_id}_task_{i:04d}" for i in range(5)]:
        ...
        result = await self.make_request("POST", f"/v1/rounds/{round_id}/task-responses", data)
        self.log_test("Submit Task Responses", result)
        return result["success"]
    
    async def test_evaluate_tasks(self, round_id: str):
        ...
        result = await self.make_request("POST", f"/v1/rounds/{round_id}/evaluate", data)
        self.log_test("Evaluate Tasks", result)
        return result["success"]
    
    async def test_calculate_scores(self, round_id: str):
        ...
        result = await self.make_request("POST", f"/v1/rounds/{round_id}/score", data)
        self.log_test("Calculate Scores", result)
        return result["success"]
    
    async def test_assign_weights(self, round_id: str):
        ...
        result = await self.make_request("POST", f"/v1/rounds/{round_id}/assign-weights", data)
        self.log_test("Assign Weights", result)
        return result["success"]
    
    async def test_complete_round(self, round_id: str):
        ...
        result = await self.make_request("POST", f"/v1/rounds/{round_id}/complete", data)
        self.log_test("Complete Round", result)
        return result["success"]
    
    async def test_get_round_status(self, round_id: str):
        """Test getting round status."""
        params = {"validator_uid": self.validator_info["validator_uid"]}
        result = await self.make_request("GET", f"/v1/rounds/{round_id}/status", params=params)
        self.log_test("Get Round Status", result)
        return result["success"]
    
    async def test_get_round_details(self, round_id: str):
        """Test getting round details."""
        params = {"validator_uid": self.validator_info["validator_uid"]}
        result = await self.make_request("GET", f"/v1/rounds/{round_id}/details", params=params)
    ...
    async def test_complete_pipeline(self):
        ...
        # Start round
        round_id = await self.test_round_start()
        if not round_id:
            print("❌ Failed to start round, aborting pipeline test")
            return False
        
        # Generate tasks
        if not await self.test_generate_tasks(round_id):
            print("❌ Failed to generate tasks")
            return False
        
        # Distribute tasks
        if not await self.test_distribute_tasks(round_id):
            print("❌ Failed to distribute tasks")
            return False
        
        # Submit responses
        if not await self.test_submit_responses(round_id):
            print("❌ Failed to submit responses")
            return False
        
        # Evaluate tasks
        if not await self.test_evaluate_tasks(round_id):
            print("❌ Failed to evaluate tasks")
            return False
        
        # Calculate scores
        if not await self.test_calculate_scores(round_id):
            print("❌ Failed to calculate scores")
            return False
        
        # Assign weights
        if not await self.test_assign_weights(round_id):
            print("❌ Failed to assign weights")
            return False
        
        # Complete round
        if not await self.test_complete_round(round_id):
    ...
    async def test_round_queries(self, round_id: str):
        """Test round query endpoints."""
        print(f"\n🔍 Testing Round Query Endpoints for {round_id}")
        print("=" * 50)
        
        await self.test_get_round_status(round_id)
        await self.test_get_round_details(round_id)
        ...
        if pipeline_success:
            ...
            for result in self.test_results:
                if result["test"] == "Start Round" and result["success"]:
                    round_id = result["data"].get("data", {}).get("round_id")
                    break
            
            if round_id:
                await self.test_round_queries(round_id)
