...
async def _save_tasks_optimized(db, tasks, round_data, validator_uid):
    ...
    for task in tasks:
        ...
        task_doc["round_id"] = round_data.round_id
...
async def _save_agent_runs_optimized(db, agent_runs, round_data, validator_uid):
    ...
    for agent_run in agent_runs:
        ...
        agent_run_doc["round_id"] = round_data.round_id
...
async def _save_task_solutions_optimized(db, task_solutions, round_data, validator_uid):
    ...
    for task_solution in task_solutions:
        ...
        solution_doc["round_id"] = round_data.round_id
...
async def _save_evaluation_results_optimized(db, evaluation_results, round_data, validator_uid):
    ...
    for eval_result in evaluation_results:
        ...
        eval_doc["round_id"] = round_data.round_id
...
async def submit_round_data_optimized(payload: RoundSubmissionRequest):
    ...
    try:
        logger.info(f"Starting optimized round submission for round {payload.round.round_id}")
        ...
        logger.info(f"Saved round {payload.round.round_id}")
        ...
        response = RoundSubmissionResponse(
            success=True,
            message=f"Successfully submitted round {payload.round.round_id} (optimized)",
        ...
        logger.info(f"Optimized round submission completed in {processing_time:.3f}s for round {payload.round.round_id}")
        return response
