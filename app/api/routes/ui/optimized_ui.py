        ...
        if recent_rounds:
            ...
            if latest_round.get("winners"):
                ...
                live_events.append({
                    ...
                    "message": f"Round {latest_round['round_id']} completed with top score {top_winner.get('score', 0.0):.2f}"
                })
...
async def get_optimized_leaderboard(
    ...
    try:
        ...
        leaderboard_data_obj = LeaderboardData(
            ...
            limit=limit,
            offset=skip,
            sort_by="score" if type == "miners" else "round_id",
            sort_order="desc"
        )
...
async def get_optimized_agent_run_details(agent_run_id: str):
    ...
    try:
        ...
        agent_run_info = {
            ...
            "round_id": agent_run_details["round_id"],
...
async def get_optimized_task_details(task_id: str):
    ...
    try:
        ...
        round_summary = await OptimizedDataBuilder.get_round_summary(task_details["round_id"])
        
        if not round_summary:
            raise HTTPException(status_code=404, detail=f"Round {task_details['round_id']} not found")
        
        # Convert to UI format
        task_info = {
            "task_id": task_details["task_id"],
            "round_id": task_details["round_id"],
