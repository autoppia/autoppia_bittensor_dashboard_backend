    ...
    try:
        ...
        if rounds:
            ...
            live_events.append({
                ...
                "message": f"Round {latest_round.validator_round_id} completed with top score {current_top_score:.2f}"
            })
...
async def get_task_details(task_id: str):
    ...
    try:
        ...
        round_doc = await db.rounds.find_one({"validator_round_id": task_doc["validator_round_id"]})
        if round_doc is None:
            raise HTTPException(status_code=404, detail=f"Round {task_doc['validator_round_id']} not found")
        
        # Convert to UI format
        task_info = {
            "task_id": task_doc["task_id"],
            "validator_round_id": task_doc["validator_round_id"],
