...
async def get_rounds(
    ...
    try:
        ...
        if status:
            rounds_data = [r for r in rounds_data if (r.validator_round_id == "round_020" and status == "active") or (r.validator_round_id != "round_020" and status == "completed")]
        
        # Sort rounds
        if sortBy == "id":
            rounds_data.sort(key=lambda x: int(x.validator_round_id.split('_')[1]) if '_' in x.validator_round_id else 0, reverse=(sortOrder == "desc"))
        ...
        for round_data in paginated_rounds:
            ...
            rounds.append({
                "id": int(round_data.validator_round_id.split('_')[1]),
                "startBlock": int(start_block),
                "endBlock": int(end_block),
                "current": round_data.validator_round_id == "round_020",  # Make round 20 current
...
@router.get("/{validator_round_id}", response_model=RoundDetailResponse)
@cached("round_detail", CACHE_TTL["round_detail"])
async def get_round_detail(validator_round_id: int = Path(..., description="Round ID")):
    """
    Returns detailed information for a specific round.
    """
    try:
        logger.info(f"Fetching round detail for validator_round_id={validator_round_id}")

        # Get round data directly from mock DB (much faster)
        from app.db.mock_mongo import get_mock_db
        from app.models.schemas import Round
        db = get_mock_db()
        round_doc = await db.rounds.find_one({"validator_round_id": f"round_{validator_round_id:03d}"})
        if not round_doc:
        ...
        round_info = {
            "id": validator_round_id,
            "startBlock": int(start_block),
            "endBlock": int(end_block),
            "current": round_data.validator_round_id == "round_020",
            "startTime": datetime.fromtimestamp(round_data.started_at, tz=timezone.utc).isoformat(),
            "endTime": datetime.fromtimestamp(round_data.ended_at, tz=timezone.utc).isoformat() if round_data.ended_at else None,
            "status": "active" if round_data.validator_round_id == "round_020" else "completed",
            "totalTasks": round_data.n_tasks,
...
@router.get("/{validator_round_id}/statistics", response_model=RoundStatisticsResponse)
@cached("round_statistics", CACHE_TTL["round_statistics"])
async def get_round_statistics(validator_round_id: int = Path(..., description="Round ID")):
    """
    Returns comprehensive statistics for a specific round.
    """
    try:
        logger.info(f"Fetching statistics for validator_round_id={validator_round_id}")
...
@router.get("/{validator_round_id}/miners", response_model=RoundMinersResponse)
@cached("round_miners", CACHE_TTL["round_miners"])
async def get_round_miners(
    validator_round_id: int = Path(..., description="Round ID"),
    page: int = Query(1, ge=1, description="Page number"),
    limit: int = Query(20, ge=1, le=100, description="Items per page"),
    ...
    """
    try:
        logger.info(f"Fetching miners for validator_round_id={validator_round_id}")
...
@router.get("/{validator_round_id}/miners/top", response_model=RoundMinersResponse)
async def get_round_top_miners(
    ...
    """
    try:
        logger.info(f"Fetching top miners for validator_round_id={validator_round_id}")
        ...
        round_data = next((r for r in rounds_data if r.validator_round_id == f"round_{validator_round_id:03d}"), None)
...
@router.get("/{validator_round_id}/miners/{uid}", response_model=RoundMinersResponse)
async def get_round_miner_detail(
    ...
    """
    try:
        logger.info(f"Fetching miner detail for validator_round_id={validator_round_id}, uid={uid}")
...
@router.get("/{validator_round_id}/validators", response_model=RoundValidatorsResponse)
@cached("round_validators", CACHE_TTL["round_validators"])
async def get_round_validators(validator_round_id: int = Path(..., description="Round ID")):
    """
    Returns validators and their performance for a specific round.
    """
    try:
        logger.info(f"Fetching validators for validator_round_id={validator_round_id}")
        ...
        for i, validator in enumerate(round_data.validators):
            ...
            import hashlib
            seed = f"{validator.uid}_{validator_round_id}"
...
@router.get("/{validator_round_id}/validators/{validator_id}", response_model=RoundValidatorsResponse)
async def get_round_validator_detail(
    ...
    """
    try:
        logger.info(f"Fetching validator detail for validator_round_id={validator_round_id}, validator_id={validator_id}")
...
@router.get("/{validator_round_id}/activity", response_model=RoundActivityResponse)
async def get_round_activity(
    ...
    """
    try:
        logger.info(f"Fetching activity for validator_round_id={validator_round_id}")
...
@router.get("/{validator_round_id}/progress", response_model=RoundProgressResponse)
async def get_round_progress(validator_round_id: int = Path(..., description="Round ID")):
    """
    Returns real-time progress information for a round.
    """
    try:
        logger.info(f"Fetching progress for validator_round_id={validator_round_id}")
...
@router.get("/{validator_round_id}/summary", response_model=RoundSummaryResponse)
async def get_round_summary(validator_round_id: int = Path(..., description="Round ID")):
    """
    Returns quick summary statistics for a round.
    """
    try:
        logger.info(f"Fetching summary for validator_round_id={validator_round_id}")
...
async def compare_rounds(request: RoundComparisonRequest):
    ...
    try:
        ...
        comparisons = []
        for validator_round_id in request.roundIds:
            round_data = next((r for r in rounds_data if r.validator_round_id == f"round_{validator_round_id:03d}"), None)
...
@router.get("/{validator_round_id}/timeline", response_model=RoundTimelineResponse)
async def get_round_timeline(validator_round_id: int = Path(..., description="Round ID")):
    """
    Returns timeline data showing progress over time.
    """
    try:
        logger.info(f"Fetching timeline for validator_round_id={validator_round_id}")
