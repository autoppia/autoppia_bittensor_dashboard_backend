...
async def get_round(validator_round_id: str):
    """
    ...
    Args:
        validator_round_id: The ID of the round to retrieve
    ...
    """
    try:
        logger.info(f"Fetching round {validator_round_id}")
        ...
        if not round_doc:
            ...
            raise HTTPException(status_code=404, detail=f"Round {validator_round_id} not found")
        ...
        logger.info(f"Successfully retrieved round {validator_round_id}")
        return round_with_details
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error fetching round {validator_round_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to fetch round: {str(e)}")


@router.get("/{validator_round_id}/agent-runs", response_model=List[AgentEvaluationRunWithDetails])
async def get_round_agent_runs(
    ...
    Args:
        validator_round_id: The ID of the round
    ...
    """
    try:
        logger.info(f"Fetching agent runs for round {validator_round_id} with limit={limit}, skip={skip}")
        
        # Get agent runs directly from mock DB (much faster)
        from app.db.mock_mongo import get_mock_db
        from app.models.schemas import AgentEvaluationRun
        db = get_mock_db()
        agent_runs_docs = await db.agent_evaluation_runs.find({"validator_round_id": validator_round_id}).skip(skip).limit(limit).to_list(length=limit)
        ...
        for doc in agent_runs_docs:
            ...
            agent_run_with_details = AgentEvaluationRunWithDetails(
                ...
                validator_round_id=agent_run_data.validator_round_id,
        ...
        logger.info(f"Successfully retrieved {len(agent_runs)} agent runs for round {validator_round_id}")
        return agent_runs
        
    except Exception as e:
        logger.error(f"Error fetching agent runs for round {validator_round_id}: {str(e)}")
...
async def list_agent_runs(
    validator_round_id: Optional[str] = Query(default=None, description="Filter by round ID"),
    limit: int = Query(default=100, ge=1, le=1000, description="Maximum number of agent runs to return"),
    skip: int = Query(default=0, ge=0, description="Number of agent runs to skip for pagination")
):
    """
    ...
    """
    try:
        logger.info(f"Fetching agent runs list with validator_round_id={validator_round_id}, limit={limit}, skip={skip}")
        ...
        if validator_round_id:
            query_filter["validator_round_id"] = validator_round_id
