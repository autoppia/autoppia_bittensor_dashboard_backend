...
async def start_round(payload: StartRoundRequest):
    ...
    logger.info("Started validator round %s for round %s", payload.validator_round_id, payload.round.validator_round_id)
    return {"message": "Validator round created", "validator_round_id": payload.validator_round_id, "validator_round_id": payload.round.validator_round_id}
...
async def set_tasks(validator_round_id: str, payload: SetTasksRequest):
    ...
    for task in payload.tasks:
        if task.validator_round_id != round_doc["validator_round_id"]:
            raise HTTPException(
...
async def start_agent_run(validator_round_id: str, payload: StartAgentRunRequest):
    ...
    if agent_run.validator_round_id != round_doc["validator_round_id"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def add_evaluation(validator_round_id: str, agent_run_id: str, payload: AddEvaluationRequest):
    ...
    expected_validator_round_id = round_doc["validator_round_id"]
    ...
    if solution.validator_round_id != expected_validator_round_id or evaluation.validator_round_id != expected_validator_round_id:
...
async def finish_round(validator_round_id: str, payload: FinishRoundRequest):
    ...
    logger.info(
        ...
        validator_round_id,
        round_doc.get("validator_round_id"),
    )
