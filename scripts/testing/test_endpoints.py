...
class EndpointTester:
                ...
                async with session.get(url) as response:
                    ...
                    if response.status == 200:
                        ...
                        if response_data:
                            ...
                            print(f"   First Round ID: {first_round.get('round_id', 'N/A')}")
    ...
    async def test_get_round_by_id(self, round_id: str) -> Dict[str, Any]:
        """Test the GET /v1/rounds/{round_id} endpoint."""
        import aiohttp
        
        url = f"{self.base_url}/v1/rounds/{round_id}"
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(url) as response:
                    ...
                    if response.status == 200:
                        print(f"   ✅ Success!")
                        print(f"   Round ID: {response_data.get('round_id', 'N/A')}")
    ...
    async def run_comprehensive_test(self):
        ...
        # Test specific round
        if submissions:
            round_id = submissions[0].round.round_id
            print(f"\n🎯 Testing specific round: {round_id}")
            result = await self.test_get_round_by_id(round_id)
            test_results["get_tests"].append({"endpoint": f"round_{round_id}", "result": result})
