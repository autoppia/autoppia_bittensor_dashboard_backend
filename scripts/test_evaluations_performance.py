...
Uso:
    python scripts/test_evaluations_performance.py
"""
...
async def query_by_validator_with_task(
...
async def query_by_score_range(session: AsyncSession, limit: int = 1000):
    ...
    stmt = (
        select(EvaluationORM)
        .where(EvaluationORM.eval_score >= 0.7)
        .limit(limit)
    )
    result = await session.scalars(stmt)
    return result.all()
...
async def query_aggregation(session: AsyncSession, validator_uid: int):
    ...
    stmt = select(
        ...
        func.sum(
            func.cast(EvaluationORM.eval_score >= 0.7, type_=func.Integer())
        ).label('success_count'),
    ).where(EvaluationORM.validator_uid == validator_uid)
...
async def main():
    ...
    try:
        ...
        async with tester.async_session() as session:
            total_evals = await session.scalar(
                select(func.count()).select_from(EvaluationORM)
            )
            
            # Obtener un validator_uid válido
            validator_uid = await session.scalar(
                select(EvaluationORM.validator_uid)
                .where(EvaluationORM.validator_uid.isnot(None))
                .limit(1)
            )
            
            # Obtener un task_id válido
            task_id = await session.scalar(
                select(EvaluationORM.task_id).limit(1)
            )
        ...
        # 1. Count simple
        elapsed, count = await tester.run_query(
            "1. Count total de evaluaciones",
            query_count_all
        )
        ...
        elapsed, count = await tester.run_query(
            "2. Evaluaciones recientes (100, sin JOINs)",
        ...
        elapsed, count = await tester.run_query(
            "3. Evaluaciones recientes (100, con JOINs)",
            query_evaluations_with_joins,
            100
        )
        ...
        elapsed, count = await tester.run_query(
            "4. Por validator_uid (1000, sin JOINs)",
            query_by_validator,
            validator_uid,
            1000
        )
        ...
        elapsed, count = await tester.run_query(
            "5. Por validator_uid (1000, con JOIN tasks)",
            query_by_validator_with_task,
            validator_uid,
            1000
        )
        ...
        elapsed, count = await tester.run_query(
            "7. Por score range (eval_score >= 0.7)",
        ...
        # 8. Query paginado
        elapsed, count = await tester.run_query(
            "8. Paginado (página 1, 50 por página)",
            query_paginated,
            1,
            50
        )
        ...
        elapsed, count = await tester.run_query(
            "9. Count por validator_uid",
            query_count_by_validator,
            validator_uid
        )
        ...
        # 10. Agregación
        elapsed, count = await tester.run_query(
            "10. Agregación (stats) por validator",
            query_aggregation,
            validator_uid
        )
