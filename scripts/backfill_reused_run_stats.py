...
Uso (con venv y .env/DATABASE_URL):
  python scripts/backfill_reused_run_stats.py
  python scripts/backfill_reused_run_stats.py --dry-run   # solo imprime qué se actualizaría
"""
...
async def backfill(session: AsyncSession, dry_run: bool) -> int:
    ...
    stmt = select(AgentEvaluationRunORM).where(
        AgentEvaluationRunORM.is_reused == True,
